...
Si se encuentra el patrón, la función debe finalizar mostrando en pantalla el mensaje "Ok", pero si detecta que
la frase no contiene los elementos indicados, debe informarle al usuario "La dirección de email es incorrecta" imprimiendo el mensaje.
'''
import re
'''
def verificar_email(email):
    patron = r'(\w+\@\w+\.com+\.br)|(\w+\@\w+\.com)'
...
pero si detecta que la frase no contiene "Hola", debe informarle al usuario "No has saludado" 
imprimiendo el mensaje en pantalla.
'''
import re
'''
def verificar_saludo(frase):
    resultado=re.search(r'^hola',frase)
    print(resultado)
    if resultado:
        print("Ok")
    else:
        print("No has saludado")
frase=(input("hola como estas?: ")).lower()
...
a continuación (ejemplo: XX1234). Crea una función, llamada verificar_cp para comprobar si el código postal pasado 
como argumento sigue este patrón. Si el patrón es correcto, mostrar al usuario el mensaje "Ok", de lo contrario: 
"El código postal ingresado no es correcto".
'''
import re

def verificar_cp(cp):
    patron=r'^\w{2}\d{4}$'   #con el ^ al inicio y el $ al final solo acepta esta estructura y no mas.
...
cp=(input("Ingresa tu codigo postal: ")).lower()
verificar_cp(cp)
